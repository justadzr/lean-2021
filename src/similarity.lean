...
lemma similarity_factor_sqrt_eq' {f' : E →L[ℝ] F} (h : is_conformal_map f') :
  similarity_factor_sqrt h ^ 2 = conformal_factor h :=
...
lemma similarity_factor_sqrt_eq (h : ∀ y, is_conformal_map $ f' y) :
  (λ x, (similarity_factor_sqrt $ h x) ^ 2) = (λ x, conformal_factor $ h x) :=
begin
  ext1 y, 
...
lemma similarity_factor_sqrt_inv_eq' {f' : E →L[ℝ] F} (h : is_conformal_map f') :
  (similarity_factor_sqrt_inv h)⁻¹ ^ 2 = conformal_factor h :=
...
lemma similarity_factor_sqrt_inv_eq (h : ∀ y, is_conformal_map $ f' y) :
  (λ x, (similarity_factor_sqrt_inv $ h x)⁻¹ ^ 2) = (λ x, conformal_factor $ h x) :=
begin
  ext1 y,
...
lemma similarity_factor_sqrt_inv_fderiv [nontrivial E] (x : E) (h : ∀ y, is_conformal_map $ f' y) 
  ...
  -(fderiv ℝ (λ y, similarity_factor_sqrt $ h y) x) * (λ y, (conformal_factor $ h x)⁻¹) :=
begin
...
lemma similarity_factor_inversion {E : Type*} [inner_product_space ℝ E] [nontrivial E]
  {r : ℝ} (hr : 0 < r) {x x₀ : E} (hx : x ≠ x₀) :
  conformal_factor (is_conformal_map_fderiv_inversion hr hx) = r ^ 2 * (∥x - x₀∥ ^ 4)⁻¹ :=
begin
  rcases exists_ne (0 : E) with ⟨u, hu⟩,
  have triv : ⟪u, u⟫ ≠ 0 := λ w, hu (inner_self_eq_zero.mp w),
  have key := (similarity_factor_prop $ is_conformal_map_fderiv_inversion hr hx).2 u u,
