...
example (f : ℂ → ℂ) (s : set ℂ) (h : is_open s) : 
  holomorph_pregroupoid.property f s → differentiable_on ℂ f s :=
begin
  intros h,
  exact h,
end
...
{   compatible := λ e e' he he',
    begin
      ...
      exact biholomorph_groupoid.compatible he he',
    end }
...
begin
  ...
  have minor₂ : ¬ s ⊆ {x} := 
    by_contra (λ h, minor₁ $ subset.antisymm (not_not.mp h) $ λ x' hx', 
      (set.eq_of_mem_singleton hx').symm ▸ hx),
...
{ is_crowded := begin
    have : ∀ (x : X), ∃ y, ¬ y = x := λ x,
    begin
      have : ∃ (y' : ℂ), y' ∈ (chart_at ℂ x).to_local_equiv.target ∧ ¬ y' =
        (chart_at ℂ x).to_local_equiv x :=
      begin
        rcases two_points_open (chart_at ℂ x x) (chart_at ℂ x).to_local_equiv.target 
          (chart_at ℂ x).open_target (mem_chart_target ℂ x),
        exact ⟨w, h⟩,
      end,
      rcases this with ⟨y', hy'₁, hy'₂⟩,
      refine ⟨(chart_at ℂ x).to_local_equiv.symm y', _⟩,
      by_contra w,
...
@[simp] lemma fholomorph_on.mono {f : X → ℂ} {s s': set X} {hs: is_open s} {hs' : is_open s'}
...
--
lemma fholomorph_on.continuous {f : X → ℂ} {s : set X} {h : is_open s} 
  (H : fholomorph_on f h) : continuous_on f s := H.1
lemma mholomorph.continuous {f : X → Y} (h : mholomorph f) : continuous f := h.1

@[simp] lemma fholomorph_on_const {s : set X} (h : is_open s) (c : ℂ) :
  fholomorph_on (λ (x : X), c) h :=
⟨continuous_on_const, λ ϕ hϕ, by {rw function.const_comp, exact differentiable_on_const c}⟩

@[simp] lemma mholomorph_const (c : Y) :
  mholomorph (λ (x : X), c) :=
begin
  split,
...
@[simp] lemma fholomorph_on.mul {f g : X → ℂ} {s : set X} {h : is_open s} 
  (hf : fholomorph_on f h) (hg : fholomorph_on g h) : fholomorph_on (λ (x : X), (f x) * g x) h :=
⟨continuous_on.mul hf.1 hg.1, λ ϕ hϕ, differentiable_on.mul (hf.2 ϕ hϕ) $ hg.2 ϕ hϕ⟩

@[simp] lemma fholomorph_on.add {f g : X → ℂ} {s : set X} {h : is_open s} 
  (hf : fholomorph_on f h) (hg : fholomorph_on g h) : fholomorph_on (λ (x : X), (f x) + g x) h :=
⟨continuous_on.add hf.1 hg.1, λ ϕ hϕ, differentiable_on.add (hf.2 ϕ hϕ) $ hg.2 ϕ hϕ⟩

@[simp] lemma fholomorph_on.neg {f: X → ℂ} {s : set X} {h : is_open s} 
...
lemma eq_on_local_disk_around_accumulation_pt
...
begin
  ...
  have W'subsetf : W' ⊆ f⁻¹' ψ.to_local_equiv.source := 
    set.subset.trans (set.inter_subset_right _ W) (set.inter_subset_left _ _),
  have W'subsetg : W' ⊆ g⁻¹' ψ.to_local_equiv.source := 
    set.subset.trans (set.inter_subset_right _ W) (set.inter_subset_right _ _),
  have fW'subsetsource : f '' W' ⊆ ψ.to_local_equiv.source,
  { rw ← set.image_subset_iff at W'subsetf, exact W'subsetf, },
  have gW'subsetsource : g '' W' ⊆ ψ.to_local_equiv.source,
  { rw ← set.image_subset_iff at W'subsetg, exact W'subsetg, },
  have πfdiffon : 
    differentiable_on ℂ πf (ϕ '' (ϕ.to_local_equiv.source ∩ f⁻¹' ψ.to_local_equiv.source)) :=
    hf.2 ϕ ψ (chart_mem_atlas ℂ z) (chart_mem_atlas ℂ (f z)),
  have πgdiffon : 
    differentiable_on ℂ πg (ϕ '' (ϕ.to_local_equiv.source ∩ g⁻¹' ψ.to_local_equiv.source)) :=
  ...
  have ϕballsubset : ϕball ⊆ ϕ.to_local_equiv.target := 
  λ v hv, begin
    rw local_homeomorph.image_eq_target_inter_inv_preimage ϕ (set.inter_subset_left _ _) 
      at πfϕballsubset,
  ...
  have superkeymem' : ϕ z ∈  ϕ '' (G ∩ ϕ.to_local_equiv.source) ∩ superkeyset := 
    ⟨⟨z, ⟨⟨key₁, mem_chart_source ℂ z⟩, rfl⟩⟩, superkeymem⟩,
  have subkeyaccum : accumulation_pt ℂ (ϕ '' (G ∩ ϕ.to_local_equiv.source)) (ϕ z) := 
    accumulation_pt_local_homeomorph 
      (accumulation_pt_open_inter ϕ.open_source (mem_chart_source ℂ z) hz) (mem_chart_source ℂ z),
  have superkeyaccum : accumulation_pt ℂ 
    (ϕ '' (G ∩ ϕ.to_local_equiv.source) ∩ superkeyset) (ϕ z) := 
    accumulation_pt_open_inter superkeyopen superkeymem subkeyaccum,
  ...
  begin
    ...
    have trivial₂ : ϕ.symm m ∈ G := 
      ((set.inj_on.mem_image_iff ϕ.to_local_equiv.inj_on 
      (set.inter_subset_right G ϕ.to_local_equiv.source) trivial₁).mp hm₁).1,
  ...
  have superkeymemU : z ∈ U := 
    ⟨ϕ z, ⟨mem_ball_self hε, local_homeomorph.left_inv ϕ (mem_chart_source ℂ z)⟩⟩,
  ...
  begin
    ...
    exact calc f m = (ψ.symm) (ψ $ f m) : 
        (local_homeomorph.left_inv ψ $ fW'subsetsource $ mem_image_of_mem f trivial₁).symm
...
--- Contains some redundant lemmas. Bad obj names. Will clean it up.
lemma nondiscrete_closure_of_eq_mholomorph
  {f g : X → Y} (hf : mholomorph f) (hg : mholomorph g)
...
lemma identity_lemma_of_mholomorph
  {f g : X → Y} (hf : mholomorph f) (hg : mholomorph g)
...
begin
  ...
  have key₂ : is_closed G:= 
    is_closed_of_closure_subset (λ z hz, let ⟨U, h₁, h₂, h₃⟩ := 
    nondiscrete_closure_of_eq_mholomorph hf hg key₁ this hz in ⟨U, h₁, h₂, h₃⟩),
  have key₃ : G = ∅ ∨ G = univ := is_clopen_iff.mp ⟨key₁, key₂⟩,
  have key₄ : G.nonempty := let ⟨U, h₁, h₂, h₃⟩ := 
    eq_on_local_disk_around_accumulation_pt hf hg hS hs₀ hs₀' in ⟨s₀, ⟨U, h₁, h₂, h₃⟩⟩,
...
lemma chart_at_origin_of_fun_at
...
--- Unfinished
lemma local_behavior_of_mholomorph
